      ...
      {step === 4 && (
        <div className="space-y-4 pt-2">
          ...
          {validRecords.length > 5 && (
            <div className="bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-sm text-sm flex items-start gap-2 shadow-sm mx-4 md:mx-0 mt-4 mb-2">
              <Warning weight="fill" className="w-5 h-5 shrink-0 mt-0.5 text-amber-500" />
              <div>
                <span className="font-semibold block mb-0.5">Demonstration Mode Active</span>
                To prevent SendGrid API rate limits, only the first 5 emails will be dispatched. The remaining records will securely generate PDFs in-memory but skip email delivery.
              </div>
            </div>
          )}

          <div className="flex justify-end gap-3 pt-2 px-4 md:px-0">
            <button 
              onClick={() => {
                setStep(1);
                setFile(null);
